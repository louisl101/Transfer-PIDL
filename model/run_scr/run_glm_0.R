...
run_PB <- function(lake_infor, job, num_cores, config_tmp_root, lswt_dir_root, met_dir_root, tmp_dir_root, output_dir_root) {
  ...
  tryCatch({
    ...
    if (!file.exists(file.path(output_dir, 'glm3.json'))) {
      ...
      write_csv(standards[[2]], 'wtemp_profile_standard.csv')
      write_configs(config_file, lake_infor, sim_date_start, vali_date_end)
      export_config(config_file = config_file, model = model)
      run_ensemble(config_file = config_file, model = model,
                         parallel = FALSE, ncores = ncores, return_list = FALSE)
      ncdf_dir <- file.path("output/glm_output.nc")
...
output_dir_root <- file.path(root_dir, 'result', 'PB_0')
...
# Set up the future plan for parallel execution (using multisession for cross-platform compatibility)
plan(multisession, workers = num_cores)

# Run the simulation in parallel using future_lapply (without progress bar)
results <- future_lapply(lake_info_list, function(lake_infor) {
  run_PB(
