...
run_PB <- function(lake_infor, ncores, fs, iter_seed, config_tmp_root, lswt_dir_root, met_dir_root, tmp_dir_root, output_dir_root) {
  result <- NULL
  lake_id <- lake_infor$LakeID
  output_dir <- file.path(output_dir_root, lake_id, paste0('fs_',fs), paste0('iter_seed_',iter_seed))
  ...
  tryCatch({
    ...
    if (!file.exists(file.path(output_dir, 'glm3.json'))) {
      ...
      observed_lswt <- read_csv(file.path(lswt_dir_root, paste0('lake_id_', lake_id, '.csv')),show_col_types = FALSE)
      ...
      cal_lswt <- cal_lswt[!is.na(cal_lswt$lake_surface_water_temperature), ] # retain valid data
      set.seed(iter_seed)
      cal_lswt <- cal_lswt[sample(nrow(cal_lswt), fs), ]  # random sampling
      cal_lswt <- cal_lswt[order(cal_lswt$Date), ]  # re-order
      #
      ...
      cali_res <- cali_ensemble(config_file = config_file, num = 150, cmethod = "LHC",
      ...
      lswt <- lswt[order(lswt$Date), ]  # re-order
      standards <- generate_LER_standard(lake_infor, meteo_file, lswt)
      write_csv(standards[[2]], 'wtemp_profile_standard.csv')
      write_configs(config_file, lake_infor, sim_date_start, vali_date_end, best_p)
      export_config(config_file = config_file, model = model)
      run_ensemble(config_file = config_file, model = model,
...
# root_dir <- '/Users/ml/Documents/ML-data/9-CODES/PycharmProjects/trans_PIML'
...
lake_name    <- ifelse(length(args) >= 1, args[1], "exper1_01")
...
tmp_dir_root <- ifelse(length(args) >= 5, args[5], tmp_dir_default)
# Print the parameters for confirmation
cat("ncores:", ncores, "\n")
cat("fs:", fs, "\n")
cat("iter_seed:", iter_seed, "\n")
cat("tmp_dir:", tmp_dir_root, "\n")
## --------------------------base file path ----------------------------##
model <- "GLM"
data_dir_root <- file.path(root_dir, 'data', 'insitulakes')
lake_list <- read_csv(file.path(data_dir_root, 'lake_info', 'exper1_target.csv'), show_col_types = FALSE)
...
output_dir_root <- file.path(root_dir, 'result', 'PB', 'exper1' , 'sample')
...
lake_infor <- subset(lake_list, LakeID == lake_name)
run_PB(
