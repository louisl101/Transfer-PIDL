...
if __name__ == "__main__":
    ...
    mdl_dir = os.path.join(root_dir, 'result', 'trans_PIDL', 'finetune', 'exper2')
    lake_list = pd.read_csv(os.path.join(insitu_dir, 'lake_info', 'exper2_target.csv'))
    output_dir = os.path.join(root_dir, 'result', 'trans_PIDL', 'projection')
    ...
    for i in trange(len(lake_list)):
        ...
        try:
            ...
            mdl = load_model(os.path.join(mdl_dir, lake_id, f'cal_model.pt'))
            ...
            demask_df = data_set.query('Mask_obs==1').reset_index(drop=True)
            cal_idx, val_idx = data_split(demask_df, test_size=0.4, shuffle=False)
            split_date = cal_idx.Date.values[-1]
