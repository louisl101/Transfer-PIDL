...
output_dir = os.path.join(root_dir, 'result','PB','exper1', 'sample')
# Path to the Python script you want to run (update filename if different)
R_script = os.path.join(root_dir, 'model', 'run_scr', "run_glm_sample.R")
task_ids = ['exper1_01', 'exper1_02', 'exper1_03']
fine_tune_sizes = [i*30 for i in [1, 3, 6, 12, 24, 48]]  # number or fraction of points to retain for fine-tuning (< 0 for all, 0-1 for percentage, >= 1 absolute number)
iter_seeds = [10, 20, 30, 40, 50]  # seed to fix the random sampling of pretrain_scale and fine_tune_size, useless during fine-tuning
aggregated_results = []
for task_id in task_ids:
    for fs in fine_tune_sizes:
        for iter_seed in iter_seeds:
            ...
            aggregated_csv_path = os.path.join(output_dir, "PB_sample.csv")
