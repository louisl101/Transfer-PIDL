...
def run_task(Configs, Search_space, data_packs, output_dir):
    # unpack
    Configs |= dict(physics_informed=True,
                    freeze=False
                    )
    Search_space |= dict(Epoch=[400])
    ground_data_pack, synthetic_data_pack = data_packs
    # pretrain on PB synthetic data
    pre_CV_result, pre_cal_result, pretrain_model, _ = run_cali(Configs, Search_space, synthetic_data_pack,pretrain_model=None)
    # finetune on ground datg
    Configs |= dict(freeze=False)
    Search_space |= dict(Epoch=[200])
    CV_result, cal_result, cal_model, cal_generations = run_cali(Configs, Search_space, ground_data_pack,pretrain_model=pretrain_model)
    ## save pretrain
    pre_CV_result.to_csv(os.path.join(output_dir, "pre_CV_result.csv"), index=False)
    save_model(pretrain_model, -1, os.path.join(output_dir, "pretrain_model.pt"))
    pre_cal_result.to_csv(os.path.join(output_dir, "pre_cal_result.csv"), index=False)
    ## save finetune
...
if __name__ == '__main__':
    ...
    Search_space = {
        ...
        "hidden_dim_fc1": [32],
        "hidden_dim_fc2": [12],
        "lam_ec": [1e-1, 1e-2, 1e-3]
    }
    expers = ['exper1', 'exper2', 'exper3']
    for exper in expers:
        Configs |= dict(exper=exper)
        if exper == 'exper1':
            Search_space |= dict(learning_rate=[5e-3])
